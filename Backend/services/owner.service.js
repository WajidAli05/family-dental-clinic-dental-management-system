...
/**
 * SKU generation now lives in services/shared/inventory.js (`nextInventorySku`)
 * — this used to be a second, independent copy of the exact fragile pattern
 * that caused the lab case E11000 (createdAt sort, check-then-act, Date.now()
 * fallback). It is also the ONLY generator now: receptionist's create used to
 * accept a client-supplied `sku` and never call any generator at all, which
 * is why new receptionist items showed a blank SKU. Both roles now call the
 * same function, so a SKU can never diverge or go ungenerated by role.
 */
...
export async function ownerInventoryListItems(_ownerId, { page, limit, sortBy, sortDir, q } = {}) {
  ...
  // Derived on read from rows already fetched, same as the lab-case overdue
  // sweep — no scheduler, capped per call, de-duplicated in the sweep itself.
...
export async function ownerInventoryUpdateStock(_ownerId, itemPublicId, payload = {}) {
  ...
  // Shared math — see computeStockAdjustment for the validation this closes
  // (an empty quantity previously coerced to 0 and silently succeeded).
  doc.qty = computeStockAdjustment(doc.qty, payload.mode, payload.qty);
