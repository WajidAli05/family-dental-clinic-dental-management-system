...
const toUiItem = (x, today = "") => {
  ...
  return {
    id: core.id,
    // Regression from the last refactor: `sku` was never copied into this
    // DTO, so the receptionist table showed blank SKUs even for items that
    // had one (created by the owner, or before this session's fix).
    sku: core.sku,
    name: core.name,
...
export async function receptionistListInventory(_receptionistId, { q, stockFilter, page, limit, sortBy, sortDir } = {}) {
  ...
  // Same derive-on-read sweep as the owner side — de-duplicated inside the
  // sweep itself, so both roles loading this page repeatedly inserts nothing
  // extra.
  await sweepInventoryThresholdNotifications(rows);
...
export async function receptionistCreateInventoryItem(_receptionistId, body) {
  ...
  // SKU is always backend-generated — never accepted from the client. This
  // was the actual cause of blank SKUs on receptionist-created items: this
  // create path used to take `body?.sku` verbatim (defaulting to "") and
  // never called any generator at all.
  const sku = await nextInventorySku();
...
export async function receptionistUpdateInventoryItem(_receptionistId, itemPublicId, body) {
  ...
  // update fields safely — SKU is intentionally excluded: it is
  // backend-generated at create time and never editable from the client,
  // matching the owner side's existing convention.
...
/**
 * Stock adjustment — the receptionist previously had no equivalent to the
 * owner's add/subtract/set flow at all. Uses the SAME computeStockAdjustment
 * as ownerInventoryUpdateStock, so the two roles' quantity math cannot
 * diverge (one shared path per the repeated divergent-copy history in this
 * codebase).
 */
export async function receptionistUpdateInventoryStock(_receptionistId, itemPublicId, payload = {}) {
  const item = await InventoryItem.findOne({ publicId: itemPublicId });
  if (!item) throw new Error("Item not found");

  item.qty = computeStockAdjustment(item.qty, payload.mode, payload.qty);
