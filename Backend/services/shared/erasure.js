 ...
 *   - Does not delete the Patient document, Invoices, Appointments, or
...
export async function erasePatientPII(req, patientPublicId) {
  const patient = await Patient.findOne({ publicId: patientPublicId });
  ...
  const prescriptions = await Prescription.find({ patientId: patientPublicId });
