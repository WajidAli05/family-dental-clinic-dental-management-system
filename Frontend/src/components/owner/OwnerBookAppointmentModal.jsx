...
const OwnerBookAppointmentModal = ({ open, onOpenChange, onSuccess }) => {
  ...
  const searchPatients = useCallback(async () => {
    ...
    setPatient(null);
    try {
      const res = await ownerApi.listPatients({ q: query.trim(), limit: 10 });
      setResults(res.data || []);
    } catch (e) {
  ...
  const handleSubmit = async () => {
    if (!form.dentistId) return toast.error("Select a dentist");
    ...
    setSubmitting(true);
    try {
      await ownerApi.createAppointment({
      ...
      <DialogContent className="max-w-lg">
        ...
        {patient && (
          <Card className="p-3 bg-gray-50 flex items-start gap-3">
            ...
            <div className="text-sm flex-1">
              ...
              <p className="text-gray-500 flex items-center gap-1"><Phone size={12} /> {patient.phone}</p>
            </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="col-span-2 space-y-1">
            <Label>Reason</Label>
            <Input placeholder="Consultation..." value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} disabled={submitting} />
          </div>
          <div className="col-span-2 space-y-1">
            <Label>Notes</Label>
            <Input placeholder="Optional notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} disabled={submitting} />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <Button
            ...
            disabled={submitting || !patient || !form.dentistId}
            className="bg-[#2ec4b6] hover:bg-[#26a699]"
          >
...
export default OwnerBookAppointmentModal;
