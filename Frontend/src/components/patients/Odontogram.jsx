 ...
 * same tooth dialog — one tooth UI, no second selector. That clinical data
 * belongs to the VISIT (Prescription.toothEntries).
 *
 * Chart-clinical mode (`chartClinical`, used by owner + receptionist): the
 * same clinical fields, but sourced from and saved back to the tooth's own
 * chart entry (Patient.odontogram) rather than a prescription. Owner passes
 * `editable` to edit them; receptionist omits it and gets read-only.
 *
 * Any tooth can always be OPENED to read its existing detail; `editable`
...
const Odontogram = ({
  ...
  const openTooth = (tooth) => {
    ...
    // clinical (prescribing): they live on the visit's prescription entry.
    const source = chartClinical ? existing : clinicalByTooth.get(tooth);
    setClin({ ...EMPTY_CLINICAL, ...(source || {}) });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Tooth status/note persists to Patient.odontogram via the caller. In
      // chartClinical mode the clinical fields ride along on the same save.
      await onSave?.(activeTooth, {
        condition: form.condition,
        note: form.note.trim(),
        ...(chartClinical ? {
          diagnosis: (clin.diagnosis || "").trim(),
          treatment: (clin.treatment || "").trim(),
          clinicalFinding: (clin.clinicalFinding || "").trim(),
          xrayRequested: Boolean(clin.xrayRequested),
          xrayNote: (clin.xrayNote || "").trim(),
        } : {}),
      });
      setActiveTooth(null);
    } finally {
  ...
  const hasClinicalContent = (e) =>
    Boolean(e && (e.diagnosis || e.treatment || e.clinicalFinding || e.xrayRequested || e.xrayNote));
  const activeHasClinical = chartClinical
    ? hasClinicalContent(byTooth.get(activeTooth))
    : Boolean(clinicalByTooth.get(activeTooth));
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Teeth carrying a clinical record — sourced from the prescription in
          clinical mode, from the chart itself in chartClinical mode. */}
      {(clinical || chartClinical) && (() => {
        const listed = chartClinical
          ? (odontogram || []).filter(hasClinicalContent)
          : toothEntries;
        return (
          <div className="rounded-lg bg-[#2ec4b6]/5 border border-[#2ec4b6]/20 px-3 py-2">
            <p className="text-xs font-semibold text-gray-700 flex items-center gap-1.5">
              ...
              {chartClinical
                ? t("patients.teethWithClinicalRecord", { count: listed.length })
                : t("patients.teethInPrescription", { count: listed.length })}
            </p>
            {listed.length > 0 && (
              ...
              {(clinical || chartClinical) && (
                <div className="space-y-3 pt-3 border-t border-gray-100">
                  <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">
                    {chartClinical
                      ? t("patients.toothClinicalRecord")
